...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		x1 = new CopyOnWriteArrayList<Integer>();
		y1 = new CopyOnWriteArrayList<Integer>();
		weightArr = new CopyOnWriteArrayList<Integer>();// 客户需求量
		...
		while (!isEnd) {
			...
			if (items[0].equals("G")) {
				...
				if ((curX != x1.get(idxNextPoint)) || (curY != y1.get(idxNextPoint))) {
					// 路上,前往下一个路线序列点
			...
			if ( Main.curType == 2 ) {
				...
				for (int i = 0; i < len; i++) {
					...
					try {
						...
						Main.nextSupRouter = i;
						Main.nextSubRouter = 2;
						if (!(weightArr.get(idxTmp) == -100 && Main.x1.get(idxTmp) == Main.xStore && Main.y1.get(idxTmp) == Main.yStore) && Main.x1.get(idxTmp) != curX && Main.y1.get(idxTmp) != curY ) {
							// 非仓库且禁止原地打转
...
class VehicleRoutingProblem {
	...
	void initData(int clientNUM, double px[], double[] py, double[] weights, int capacity) {
		...
		for (int v = 2; v <= K; v++) {
			vehicleInfoMatrix[v][0] = Main.MaxCapacity;
	...
	double caculateFitness(int[] Gh) {
		...
		for (j = 1; j < clientNum; j++) {
			...
			if (cur_q > vehicleInfoMatrix[i][0] || cur_d + distanceMatrix[Gh[j]][0] > vehicleInfoMatrix[i][1])// 还得加上返回配送中心距离
			{
